...
__attribute__((noinline)) void perf_matrix_multiply(volatile uint32 *result)
{
    uint32 i, j, k;
    uint32 sum = 0;
...
__attribute__((noinline)) void perf_crc32_compute(const uint8 *data, uint32 len, volatile uint32 *result)
{
...
__attribute__((noinline)) void perf_bubble_sort(uint32 *arr, uint32 len)
{
    uint32 i, j, tmp;
    for (i = 0; i < len - 1; i++) {
        for (j = 0; j < len - i - 1; j++) {
...
__attribute__((noinline)) void perf_memory_stress(volatile uint32 *dst, const uint32 *src, uint32 count)
{
    uint32 i;
    for (i = 0; i < count; i++) {
...
__attribute__((noinline)) void perf_bitfield_stress(volatile uint32 *result, uint32 iterations)
{
...
__attribute__((noinline)) uint32 perf_fibonacci_iterative(uint32 n)
{
    uint32 a = 0, b = 1, c, i;
    if (n == 0) return a;
    for (i = 2; i <= n; i++) {
...
__attribute__((noinline)) void perf_run_all_workloads(void)
{
